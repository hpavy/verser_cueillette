...
def recettes(nom_recette):
    # Attention il faut ajouter les recettes avec des tirets du bas !!!
    # Il ne faut pas mettre d'accents sur les noms de recettes !!!!
    version()
    if request.method == "POST" :
        donnes = request.form
...
def upload(number):
  ...
  # Enregistrez le fichier dans le dossier `/uploads`
    file.save(f"api/static/img/uploads/photos/photo{number}.jpeg")  
...
def upload_recette(nom_recette):
  ...
  # Enregistrez le fichier dans le dossier `/uploadss` 
    file.save(f"api/static/img/uploads/recettes/{nom_recette}.jpeg")  
